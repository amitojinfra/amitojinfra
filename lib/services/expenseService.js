...
class ExpenseService {
  ...
  async createExpense(expenseData, userId) {
    try {
      ...
      const docRef = await firestoreService.addDocument(this.collectionName, expense);
  ...
  async getExpenseById(expenseId) {
    try {
      const expense = await firestoreService.getDocument(this.collectionName, expenseId);

      if (expense) {
        return {
          success: true,
          expense: this.formatExpenseData(expense)
        };
      } else {
        return {
          success: false,
  ...
  async getExpenses(filters = {}) {
    try {
      ...
      if (filters.paymentMode) {
        whereConditions.push({ field: 'paymentMode', operator: '==', value: filters.paymentMode });
      }
      ...
      // Note: Date range filtering will be done client-side for now due to Firestore limitations
      // with multiple where clauses and ordering
      ...
      const documents = await firestoreService.getDocuments(this.collectionName, options);
      
      // Apply date filtering client-side
      let expenses = documents.map(doc => this.formatExpenseData(doc));
  ...
  async updateExpense(expenseId, updateData, userId) {
    try {
      ...
      await firestoreService.updateDocument(this.collectionName, expenseId, updates);
  ...
  async deleteExpense(expenseId) {
    try {
      await firestoreService.deleteDocument(this.collectionName, expenseId);
