...
class AttendanceService {
  ...
  async markAttendance(attendanceData) {
    try {
      ...
      if (existingAttendance) {
        ...
        const updatedRecord = await this.updateAttendance(attendanceId, attendanceData);
        return {
          ...updatedRecord,
          isUpdate: true,
          operation: 'updated'
    ...
    try {
      ...
      for (let i = 0; i < attendanceDataArray.length; i++) {
        try {
          ...
          // Track whether record was created or updated
  ...
  /**
   * Check if an employee has attendance marked for a specific date
   * @param {string} employeeId - Employee ID
   * @param {string} date - Date in YYYY-MM-DD format
   * @returns {Promise<Object|null>} Attendance record if exists, null otherwise
   */
  async checkEmployeeAttendanceForDate(employeeId, date) {
    try {
      const attendanceId = generateAttendanceId(employeeId, date);
      return await this.getAttendanceById(attendanceId);
    } catch (error) {
      console.error('Error checking employee attendance for date:', error);
      return null;
    }
  }

  /**
   * Get employees with existing attendance for a specific date
   * @param {Array} employeeIds - Array of employee IDs to check
   * @param {string} date - Date in YYYY-MM-DD format
   * @returns {Promise<Set>} Set of employee IDs that have attendance marked
   */
  async getEmployeesWithAttendanceForDate(employeeIds, date) {
    try {
      const employeesWithAttendance = new Set();
      
      // Check each employee's attendance for the date
      await Promise.all(
        employeeIds.map(async (employeeId) => {
          const attendance = await this.checkEmployeeAttendanceForDate(employeeId, date);
          if (attendance) {
            employeesWithAttendance.add(employeeId);
          }
        })
      );
      ...
      console.error('Error getting employees with attendance for date:', error);
      return new Set();
    }
  }
